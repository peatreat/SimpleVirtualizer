...
class CmovbInstruction : Instruction {
public:
    void execute(VmData* vm_data) {
        ...
        if (vm_data->eflags.CF == 1)
            cmovb(result.op1, result.op2, result.operation_size);

        vm_data->registers.ip += instruction_size;
    }

private:
    void cmovb(uint8_t* dst, uint8_t* src, int op_size) {
        switch (op_size) {
