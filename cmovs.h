...
class CmovsInstruction : Instruction {
public:
    void execute(VmData* vm_data) {
        ...
        if (vm_data->eflags.SF == 1)
            cmovs(result.op1, result.op2, result.operation_size);

        vm_data->registers.ip += instruction_size;
    }

private:
    void cmovs(uint8_t* dst, uint8_t* src, int op_size) {
        switch (op_size) {
